...
const PYTORCH_MODULES = {
  ...
  'Linear': {
    ...
    params: {
      ...
      device: { type: 'select', options: ['cpu', 'cuda'], default: 'cpu', description: 'Device to place the layer on' }
    }
  },
  ...
  'Conv2d': {
    ...
    params: {
      ...
      padding_mode: { type: 'select', options: ['zeros', 'reflect', 'replicate', 'circular'], default: 'zeros', description: 'Padding mode' }
    }
  },
  ...
  'MaxPool2d': {
    ...
    params: {
      ...
      stride: { type: 'number', default: 2, min: 1, max: 50, description: 'The stride of the window' },
  ...
  'CrossEntropyLoss': {
    ...
    outputs: ['loss'],
    params: {
      weight: { type: 'text', default: 'None', description: 'A manual rescaling weight given to each class' },
...
export function ImprovedVisualEditor() {
  ...
  const handleNodeMouseDown = (e: React.MouseEvent, node: BlockNode) => {
    if ((e.target as Element).classList.contains('port')) {
      return;
    }
  ...
  }, [isDragging, selectedNode, dragOffset, dragConnection]);
  ...
  // 포트에서 드래그 시작
  const handlePortMouseDown = (e: React.MouseEvent, nodeId: string, port: string, type: 'input' | 'output') => {
    e.stopPropagation();
    const rect = canvasRef.current?.getBoundingClientRect();
    if (rect) {
      setDragConnection({
  ...
  // 포트에서 드래그 끝
  const handlePortMouseUp = (nodeId: string, port: string, type: 'input' | 'output') => {
    if (dragConnection && dragConnection.startType !== type && dragConnection.startNodeId !== nodeId) {
      const newConnection: Connection = {
        id: `conn-${Date.now()}`,
        fromNodeId: dragConnection.startType === 'output' ? dragConnection.startNodeId : nodeId,
        toNodeId: dragConnection.startType === 'input' ? dragConnection.startNodeId : nodeId,
        fromPort: dragConnection.startType === 'output' ? dragConnection.startPort : port,
        toPort: dragConnection.startType === 'input' ? dragConnection.startPort : port
      };
  ...
  const generatePythonCode = () => {
    ...
    // 연결 관계를 기반으로 forward 함수 생성
    const sortedNodes = topologicalSort(nodes, connections);
    sortedNodes.forEach((node, index) => {
      if (node.type === 'Input') {
        code += "        # Input layer\n";
      } else {
        const layerName = `layer_${nodes.indexOf(node)}`;
  ...
  const topologicalSort = (nodes: BlockNode[], connections: Connection[]) => {
    // 간단한 위상 정렬 구현
    return nodes.filter(node => node.type !== 'Input').sort((a, b) => a.y - b.y);
  };
  ...
  const getPytorchCode = (node: BlockNode) => {
    ...
    switch (node.type) {
      ...
      case 'Sigmoid':
        return `nn.Sigmoid()`;
      case 'Softmax':
        return `nn.Softmax(dim=${config.dim})`;
      case 'MaxPool2d':
        return `nn.MaxPool2d(${config.kernel_size}, stride=${config.stride})`;
      case 'BatchNorm2d':
  ...
  return (
    <div className="improved-visual-editor">
      ...
      <div className="editor-content">
        ...
        >
          <svg className="connection-svg">
            ...
            {dragConnection && (
              <line
                x1={dragConnection.currentPos.x}
                y1={dragConnection.currentPos.y}
            ...
            >
              ...
              {node.inputs.map((input, index) => (
                <div
                  ...
                  onMouseUp={() => handlePortMouseUp(node.id, input, 'input')}
                />
              ))}
              ...
              {node.outputs.map((output, index) => (
                <div
                  ...
                  onMouseUp={() => handlePortMouseUp(node.id, output, 'output')}
                />
              ))}
            </div>
          ))}
                          ...
                          <div key={key} className="parameter">
                            ...
                            {param.type === 'number' ? (
                              <input
                                ...
                                max={param.max}
                                step={param.step || 1}
                                onChange={(e) => updateNodeConfig(selectedNode.id, key, parseFloat(e.target.value) || 0)}
                              />
                            ) : param.type === 'boolean' ? (
                              <input
                                type="checkbox"
